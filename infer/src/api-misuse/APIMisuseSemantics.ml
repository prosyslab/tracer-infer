...
and eval_binop bop e1 e2 mem =
  ...
  match bop with
  | Binop.Shiftlt | Binop.BOr ->
      let overflow =
        if v1.Val.user_input |> Dom.UserInput.is_taint then Dom.IntOverflow.top
      ...
      ; int_overflow= Dom.IntOverflow.join overflow v1.Val.int_overflow }
  | _ ->
      v2
